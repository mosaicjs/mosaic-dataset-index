...
export default class DataSetIndex extends DerivativeDataSet {
    ...
    setDataFilter(filter) { return this.updateSearchParams({ filter }); }
    ...
    updateSearchParams(params, force) {
        ...
        return this.action('params', {index: this}, function(intent) {
            let queryUpdated = setParam('_query', 'query');
            queryUpdated |= setParam('_dataFilter', 'filter');
            let schemaUpdated = setParam('_fields', 'fields');
    ...
    _runSearch() {
        ...
        if (!that._searchPromise){
            that._searchPromise = Promise.resolve();
        }
            ...
            return that.action('search', {index: this}, function(intent) {
                return that._getLunrIndex().then(function(index){
                    ...
                    if (!that._query) {
                        ...
                        let entries = index.lunr.search(that._query);
