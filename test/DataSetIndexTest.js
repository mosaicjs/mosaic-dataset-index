...
describe('DataSetIndex', function() {
    ...
    it('should be able to index and search data items', function(done){
        ...
        dataSet.setItems(data.features).then(function(){
            ...
            return index.search('Notre-Dame').then(function(resultSet){
                 expect(started)
                 expect(resultSet.length).to.be(1);
                 var res = resultSet.items[0]; 
            ...
            function search(query){
                return index.search(query).then(function(resultSet){
                    return resultSet.map(function(item, pos){
                        return item.data;
                    });
                })
            }
    ...
    it('should be able re-define indexing schema', function(done){
        ...
        function testIndex(index, query, ...ids) {
            return index.search(query).then(function(resultSet){
                expect(resultSet.length).to.eql(ids.length);
                resultSet.forEach(function(obj, pos){
                    expect(obj.id).to.eql(ids[pos]);
                });
                expect(resultSet.items).to.eql(resultSet.items);
            });
        }
